##################################################################################################################
#####merge Normal.celltype and TypeI.celltype---merge Normal.celltype and TypeI.celltype######################################
#####merge Normal.celltype and TypeI.celltype---merge Normal.celltype and TypeI.celltype######################################
#####merge Normal.celltype and TypeI.celltype---merge Normal.celltype and TypeI.celltype######################################
...
Normal.celltype
TypeI.celltype <- readRDS("TypeI_cellchat.celltype_after_cellchat.rds")
TypeI.celltype

levels(Normal.celltype@idents)
levels(TypeI.celltype@idents)
...
group.new = levels(TypeI.celltype@idents)
...
object.list <- list(Normal = Normal.celltype, TypeI = TypeI.celltype)
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "TypeI",ligand.logFC = 0.2, receptor.logFC = NULL)
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = c(1,3,4,6,7,9), targets.use = c(2,5,8), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = c(1,3,4,6,7,9), targets.use = c(2,5,8), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
gg1 + gg2
...
# Chord diagram
par(mfrow = c(1,1), xpd=TRUE)
netVisual_chord_gene(object.list[[2]], sources.use = c(1,3,4,6,7,9), targets.use = c(2,5,8),  slot.name = 'net', net = net.up, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
netVisual_chord_gene(object.list[[1]], sources.use = c(1,3,4,6,7,9), targets.use = c(2,5,8), slot.name = 'net', net = net.down, lab.cex = 0.8, small.gap = 3.5, title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))


pathways.show <- c("CD99") 
weight.max <- getMaxWeight(object.list, slot.name = 'net', attribute = pathways.show) # control the edge weights across different datasets
...
pdf(file ="CD99.pdf", width = 10, height =8)
netVisual_aggregate(object.list[[i]], signaling = pathways.show, layout = "chord", signaling.name = paste(pathways.show, names(object.list)[i]))
dev.off()
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("Normal", "TypeI")) # set factor level
plotGeneExpression(cellchat, signaling = "CD99", split.by = "datasets", colors.ggplot = T)
