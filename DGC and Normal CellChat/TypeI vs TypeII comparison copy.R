##################################################################################################################
#####merge TypeI.leiden_1 and TypeII.leiden_1---merge TypeI.leiden_1 and TypeII.leiden_1######################################
#####merge TypeI.leiden_1 and TypeII.leiden_1---merge TypeI.leiden_1 and TypeII.leiden_1######################################
#####merge TypeI.leiden_1 and TypeII.leiden_1---merge TypeI.leiden_1 and TypeII.leiden_1######################################
...
TypeI.leiden_1 <- readRDS("TypeI_cellchat.leiden_1_after_cellchat.rds")
TypeI.leiden_1
TypeII.leiden_1 <- readRDS("TypeII_cellchat.leiden_1_after_cellchat.rds")
TypeII.leiden_1

levels(TypeI.leiden_1@idents)
levels(TypeII.leiden_1@idents)
...
#group.new = levels(TypeII.leiden_1@idents)
#TypeI.leiden_1 <- liftCellChat(TypeI.leiden_1, group.new)
#levels(TypeI.leiden_1@idents)
...
#object.list <- list(Nomal = TypeI.leiden_1, TypeII = TypeII.leiden_1)
#cellchat <- mergeCellChat(object.list, add.names = names(object.list), cell.prefix = TRUE)

# Define the cell labels to lift up, if the idents are overlaped, but can't cover each other
object.list <- list(TypeI = TypeI.leiden_1, TypeII = TypeII.leiden_1)
cellchat <- mergeCellChat(object.list, add.names = names(object.list), cell.prefix = TRUE)

TypeI.leiden_1 <- liftCellChat(TypeI.leiden_1 , group.new = levels( cellchat@idents$joint) )

TypeII.leiden_1 <- liftCellChat(TypeII.leiden_1 , group.new = levels( cellchat@idents$joint) )
levels(TypeI.leiden_1@idents)
levels(TypeII.leiden_1@idents)
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "TypeII",ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "TypeI",ligand.logFC = -0.1, receptor.logFC = -0.1)
